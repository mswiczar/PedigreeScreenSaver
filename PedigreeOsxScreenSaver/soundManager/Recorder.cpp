...
#define filePath "/Users/mswiczar/Documents/Recording.caf"
...
/*
	...
	aqData.mDataFormat.mSampleRate = 44100;
	aqData.mDataFormat.mFormatID = kAudioFormatLinearPCM;
	aqData.mDataFormat.mFormatFlags kAudioFormatFlagsNativeFloatPacked
	
	int sampleSize = sizeof(Float32);
	aqData.mDataFormat.mBitsPerChannel = 8 * sampleSize;
...
result=	AudioQueueGetProperty (                                    // 2
	...
	printf("AudioQueueGetProperty result %d\n",result);
	...
	for (int i = 0; i < kNumberBuffers; ++i) {           // 1
		result= AudioQueueAllocateBuffer (                       // 2
			...
			printf("AudioQueueAllocateBuffer result %d\n",result);
		result= AudioQueueEnqueueBuffer (                        // 6
			...
			printf("AudioQueueEnqueueBuffer result %d\n",result);
	}
					...
					if ((R !=0 ) ||(G !=0 )|| (B !=0 ))
					{
						if (R !=0 )
						{
									   ...
									   y <30))
							{
								...
								//	printf("Level: %d , RGB (%d , %d , %d ) alpa= %d   %d , %d \n",y,arraydata[y].R,G,B,A,thisLevel,nextLevel);
